...
const App = {
  ...
  closeAuth() { hide('modal-auth'); },
  switchAuth(tab) {
    const isLogin = tab === 'login';
    toggleHidden('auth-login', !isLogin);
    toggleHidden('auth-signup', isLogin);
  },
  togglePw(id, btn) {
  ...
  login() {
    ...
    const pw = val('li-pw');
    if (!email || !pw) return this.showToast('Please fill in all fields', 'error');

    const users = JSON.parse(localStorage.getItem('td_users') || '{}');
    if (!users[email]) return this.showToast('No account found. Sign up first!', 'error');
    if (users[email].pw !== pw) return this.showToast('Incorrect password', 'error');

    state.user = users[email].user;
    save();
    load();
    this.closeAuth();
    this.enterApp();
  ...
  signup() {
    ...
    const pw = val('su-pw');
    if (!name || !email || !pw) return this.showToast('Please fill in all fields', 'error');
    if (!email.includes('@')) return this.showToast('Please enter a valid email', 'error');
    if (pw.length < 6) return this.showToast('Password must be at least 6 characters', 'error');

    const users = JSON.parse(localStorage.getItem('td_users') || '{}');
    if (users[email]) return this.showToast('Account already exists. Log in!', 'error');
  ...
  obNext(step) {
    if (step === 1) {
      ...
      const photo = imgEl.style.display !== 'none' ? imgEl.src : `https://images.unsplash.com/photo-1587300003388-59208cc962cb?w=400&q=80`;
  ...
  initDrag(card) {
    ...
    const onMove = (e) => {
      ...
      // stamp thresholds
      const likeT = cx > 50, nopeT = cx < -50, superT = cy < -60;
    ...
    const onEnd = () => {
      ...
      if (currentY < -100) {
        this.animateCard(card, 0); // super like
      } else if (currentX > 100) {
        this.animateCard(card, 1);
      } else if (currentX < -100) {
        this.animateCard(card, -1);
      } else {
  ...
  animateCard(card, dir) {
    ...
    setTimeout(() => {
      card.remove();
      this.handleSwipeResult(dogId, dir);
    }, 450);
  },
  ...
  navigate(view, btn) {
    ...
    if (btn) {
      ...
      // also sync the other nav (sidebar/mobile)
      document.querySelectorAll(`[data-view="${view}"]`).forEach(b => b.classList.add('active'));
    }

    // Center views (Discover, Profile)
    toggleHidden('av-discover', view !== 'discover');
    toggleHidden('av-profile', view !== 'profile');

    // Right panel / full-screen on mobile
    if (state.isMobile()) {
      // Mobile: show right panel as a kind of... we just use the center area
      // Actually on mobile, matches & messages are shown in right-panel but it's not displayed
      // So we need a different approach — show them in the center
      ...
      else if (view === 'discover') this.renderRightPanel('matches'); // show matches on right when discovering
    }
  ...
  renderMobileView(view) {
    // On mobile, matches and messages appear in the right panel, but we need to show them
    // Let's make the right panel visible as an overlay for mobile
    if (view === 'matches' || view === 'messages') {
      // Show right panel like a full screen view
      ...
      // Add a back button feeling — handled by nav
    } else {
      const rp = document.getElementById('right-panel');
      rp.style.display = '';
      rp.style.position = '';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('click', e => {
    ...
    if (!panel?.contains(e.target) && e.target !== btn1 && e.target !== btn2 && !btn1?.contains(e.target) && !btn2?.contains(e.target)) {
      panel?.classList.add('hidden');
    }
  });

  // Close mobile landing menu on outside click / scroll
  window.addEventListener('scroll', () => {
    document.getElementById('l-mobile-menu')?.classList.add('hidden');
  }, { passive: true });

  // Resize: re-handle mobile/desktop view transitions
  ...
  // Auto-enter app if already logged in
